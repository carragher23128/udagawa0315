...
function getOutDir(texPath) {
    ...
    if (outDir === '%DIR%' || outDir === '%DIR_W32%') {
        return lw_1.lw.compile.lastSteps.filter(step => step.outdir).slice(-1)[0]?.outdir ?? path.normalize(out).split(path.sep).join('/');
    }
...
function getFlsPath(texPath) {
    ...
    flsFile = path.resolve(rootDir, lw_1.lw.compile.lastSteps.filter(step => step.auxdir).slice(-1)[0]?.auxdir ?? '', fileName);
